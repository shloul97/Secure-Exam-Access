...
for($x = 0; $x < count($quizes);$x++)
{
    ...
    if($countDevices < $countUsers){
        echo json_encode(value: ['message' => 'Device out of range']);
        break;
    }
    else{
        ...
        foreach($ipsArr as $ip){
            ...
            error_log("Total Users: $countUsers");
            error_log("Total Devices: $countDevices");
            ...
            try{
                ...
                //echo json_encode(['message' => "Users distrputed to devices"], JSON_PRETTY_PRINT);
            }
            catch (Exception $e) {
                echo json_encode([
